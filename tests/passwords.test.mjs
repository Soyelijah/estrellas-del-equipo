...
test("stores a password as a salted PBKDF2 hash and verifies only the original secret", async () => {
  const encoded = await hashPassword("Una contraseña larga y privada 2026");

  assert.match(encoded, /^pbkdf2_sha256\$600000\$[A-Za-z0-9_-]+\$[A-Za-z0-9_-]+$/);
